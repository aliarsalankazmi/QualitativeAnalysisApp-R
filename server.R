...
cleanSweepCorpus <- function(corpus, useStopwords = FALSE, stem = FALSE, removePunct = FALSE, 
    removeNum = FALSE, useSynonyms = FALSE, initialWords, replacementWords, useCustomStopwords = FALSE, 
    customStopwords) {
    newCorpus <- corpus
...
shinyServer(function(input, output, session) {
    ...
    initialUnigramMatrix <- reactive({
        ...
        isolate({
            ...
            initialMatrix <- TermDocumentMatrix(corpus, control = list(weighting = function(x) weightSMART(x, 
                spec = weightingScheme)))
            rm(corpus)
    ...
    finalUnigramMatrix <- reactive({
        ...
        isolate({
            ...
            if (input$lowerFreqBound != lowerFreqRange()[1]) {
                lowerBound <- findFreqTerms(finalUnigramMatrix, round(input$lowerFreqBound), 
                  Inf)
    ...
    output$lowerFreqSliderDendro <- renderUI({
        sliderInput("dendroSize", "Set Word Frequency lower bound", min = round(lowerFreqRangeDendro()[1]), 
            max = round(lowerFreqRangeDendro()[2]), value = round(lowerFreqRangeDendro()[1]), 
            step = NULL, ticks = TRUE)
    })
    ...
    rankFrequencyPlot <- reactive({
        ...
        isolate({
            ...
            ggplot(data = myDf, aes(x = log10(rank), y = log10(freq))) + geom_text(aes(label = words, 
                size = 3, angle = 45)) + xlab("Words' Rank") + ylab("Frequency in Log scale") + 
                scale_size(guide = "none") + ggtitle("Rank-Frequency Plot")
        })
    })
    ...
    output$downloadRankFreqPlot <- downloadHandler(filename = function() {
        paste0("RankFrequencyPlot", Sys.time(), ".pdf")
    ...
    wordFrequencyPlot <- reactive({
        ...
        isolate({
            myDf <- initialDf()
            myDf <- myDf[order(myDf$freq, decreasing = TRUE), ]
            myDf$words <- factor(myDf$words, levels = myDf[order(myDf$freq, decreasing = TRUE), 
                "words"])
            ...
            ggplot(data = myDfNew, aes(x = words, y = freq)) + geom_bar(stat = "Identity") + 
    ...
    output$downloadWordFreqPlot <- downloadHandler(filename = function() {
        paste0("WordFrequencyPlot", Sys.time(), ".pdf")
    }, content = function(file) {
        pdf(file, width = 14, height = 12)
        print(wordFrequencyPlot())
        dev.off()
    ...
    docClustering <- reactive({
        ...
        isolate({
            ...
            ggplot(data = clusterData, aes(x = x, y = y)) + geom_point(aes(alpha = 0.2)) + 
                ...
                max(clusterData$x)), labels = c("", "")) + scale_y_continuous(breaks = c(min(clusterData$y), 
                max(clusterData$y)), labels = c("", ""))
        })
    })
    ...
    output$downloadDocCluster <- downloadHandler(filename = function() {
    ...
    output$downloadDendro <- downloadHandler(filename = function() {
        paste0("Dendrogram", Sys.time(), ".pdf")
    }, content = function(file) {
        pdf(file, width = 14, height = 12)
        print(dendroGraphic())
        dev.off()
    ...
    associativeCloud <- reactive({
        ...
        isolate({
            ...
            wordsDf <- data.frame(term = V(adjGraph)$name, x.pos = coordinates[, 
                1], y.pos = coordinates[, 2], affiliations = diag(myAdjMat), stringsAsFactors = FALSE)
            set.seed(1011)
            wordsKmeans <- kmeans(wordsDf[, c(2, 3)], centers = input$groupAssoc)
            
            wordsDf <- transform(wordsDf, freq = freq, percentOfDoc = 100 * (freq/length(preprocessedCorpus)), 
                groups = as.factor(wordsKmeans$cluster))
            ...
            ggplot(data = wordsDf, aes(x = x.pos, y = y.pos)) + geom_text(aes(label = term, 
                ...
                scale_size("Word Frequency") + scale_alpha("Overall Importance of Word", 
                range = c(0.4, 1), guide = "none") + scale_x_continuous(breaks = c(min(wordsDf$x.pos), 
                max(wordsDf$x.pos)), labels = c("", "")) + scale_y_continuous(breaks = c(min(wordsDf$y.pos), 
                max(wordsDf$y.pos)), labels = c("", "")) + theme(panel.grid.major = element_blank(), 
    ...
    output$downloadAssoc <- downloadHandler(filename = function() {
        paste0("AssocWordCloud", Sys.time(), ".pdf")
    }, content = function(file) {
        pdf(file, width = 14, height = 12)
        print(associativeCloud())
        dev.off()
    ...
    wordNetworkGraph <- reactive({
        ...
        isolate({
            ...
            if (input$networkSize == 0) {
                ...
                adjGraph <- delete.edges(adjGraph, E(adjGraph)[E(adjGraph)$weight <= 
                  1])
                ...
                mainDf <- data.frame(words = rownames(secondaryMat), freq = rowSums(secondaryMat), 
                  x.pos = coordinates[, 1], y.pos = coordinates[, 2], affiliations = affiliations, 
                  stringsAsFactors = FALSE)
                otherDf <- data.frame(edges1 = myEdges[, 1], edges2 = myEdges[, 2], 
                  weights = E(adjGraph)$weight, stringsAsFactors = FALSE)
                ...
                ggplot(data = finalDf, aes(x = x.pos, y = y.pos)) + geom_line(linetype = 1, 
                  ...
                  scale_alpha(guide = "none") + scale_colour_continuous(low = "#CCCC66", 
                  high = "#006600", "Weight Strength") + theme_bw() + xlab("") + 
                  ylab("") + scale_x_continuous(breaks = c(min(finalDf$x.pos), max(finalDf$x.pos)), 
                  labels = c("", "")) + scale_y_continuous(breaks = c(min(finalDf$y.pos), 
                  max(finalDf$y.pos)), labels = c("", "")) + theme(panel.grid.major = element_blank(), 
                  panel.grid.minor = element_blank(), axis.ticks = element_blank())
            } else {
                initialDf <- data.frame(words = words, freq = freq, affiliations = affiliations, 
                  stringsAsFactors = FALSE)
                impWords <- initialDf[initialDf$affiliations > quantile(initialDf$affiliations)[input$networkSize], 
                  "words"]
                reducedAdjDf <- adjacencyDf[impWords, ]
                reducedAdjDf <- reducedAdjDf[, !names(reducedAdjDf) %in% impWords]
                
                incidGraph <- graph.incidence(reducedAdjDf, mode = c("out"), multiple = TRUE, 
                  weighted = TRUE)
                incidGraph <- delete.edges(incidGraph, E(incidGraph)[E(incidGraph)$weight <= 
                  1])
                ...
                mainDf <- data.frame(words = Words, x.pos = coordinates[, 1], y.pos = coordinates[, 
                  2], stringsAsFactors = FALSE)
                wordMatch <- match(mainDf$words, initialDf$words)
                mainDf$freq <- initialDf$freq[wordMatch]
                
                otherDf <- data.frame(edge1 = Edges[, 1], edge2 = Edges[, 2], weights = Weights, 
                  stringsAsFactors = FALSE)
                otherDf$id <- row.names(otherDf)
                auxiliaryDf <- melt(otherDf, id.vars = c(3, 4))
                names(auxiliaryDf)[4] <- "words"
    ...
    output$downloadNetwork <- downloadHandler(filename = function() {
        paste0("wordsNetwork", Sys.time(), ".pdf")
    }, content = function(file) {
    ...
    observe({
        if (input$phase == "import" | input$phase == "preprocess" | input$phase == 
            "featureGenerate" | input$phase == "featureSelect") {
